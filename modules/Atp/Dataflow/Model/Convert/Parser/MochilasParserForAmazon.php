...
class Atp_Dataflow_Model_Convert_Parser_MochilasParserForAmazon extends Mage_Dataflow_Model_Convert_Parser_Csv
{
  ...
  const STORE_ADMIN = 0;
    ...
    public function unparse()
    {
        ...
        $fieldList['main-image-url'] = 'main-image-url';
        // Extra Images
        for ($i=0; $i<8; $i++) {
          $k = 'other-image-url-' . ($i+1);
          $fieldList[$k] = $k;
        }
        $fieldList['registered-parameter'] = 'registered-parameter';
        $fieldList['update-delete'] = 'update-delete';
        ...
        foreach ($batchExportIds as $batchExportId) {
            ...
            if (isset($row['sku']) && $row['sku']) {
              ...
              if ($attribute = $product->getResource()->getAttribute('image')) {
                $row['main-image-url'] = $attribute->getFrontend()->getUrl($product);
              } else {
                $row['main-image-url'] = '';
              }
              ...
              if ($mediaGallery) {
                ...
                for ($i = 0; $i < $count; $i++) {
                  $row['other-image-url-' . ($i + 1)] = $mediaGallery['items'][$i]['url'];
                }
              }
            }
